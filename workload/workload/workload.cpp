...
int main(int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("memory-size", po::value<uint64_t>(), "total memory footprint in MB")
        ("working-set-size", po::value<uint64_t>(), "working set size in MB")
        ("dirty-size", po::value<uint64_t>(), "the dirty data size to be dirtied in the period")
        ("period", po::value<uint64_t>(), "the period in millisecond")
        ("buckets", po::value<uint64_t>(), "divide the period in multiple buckets")
        ("threads", po::value<uint64_t>(), "the number of threads for running the workload")
        ("pagesize", po::value<uint64_t>(), "the pagesize (default: 4096)")
        ("cpu-util", po::value<double>(), "the target CPU utilization of the workload [0,1]")
        ("compressibility", po::value<double>(), "the compressibility of the initial data [0,1]")
        ("dirty-compressibility", po::value<double>(), "compressibility of the dirtied data [0,1]")
        ("redirtied-ratio", po::value<double>(), "ratio of redirtied pages [0,1]")
        ("delta-compressibility", po::value<double>(), "ratio of zero words in the re-dirtied data [0,1]")
        ;
